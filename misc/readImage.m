...
%                        'b_cropToBB', 'b_resizeImageToStandardSize',
%                        'i_standardImageSize'
% 
% OUTPUT:
...
% 
    ...
    if ( getFieldWithDefault ( settings, 'b_cropToBB', false ) )
        bbox = readBbox( s_imageName );
    ...
    %% (4) MAKE IMAGE ALWAYS TO BE COLOR, OR PSEUDO COLOR
    if( ndims( im ) == 2 )
        im = repmat ( im, [1,1,3] );
    end
