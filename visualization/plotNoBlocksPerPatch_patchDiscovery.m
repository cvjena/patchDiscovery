function plotNoBlocksPerPatch_patchDiscovery ( results, additionalInfos )
    ...
    if ( ( ~isfield(additionalInfos,'s_destination'))  || isempty(additionalInfos.s_destination) )
        s_destination = 'noBlocksPerPatch';
    else
    ...
    if ( ( ~isfield(additionalInfos,'s_title'))  || isempty(additionalInfos.s_title) )
        s_title = 'No. of Blocks per Patch';
    else
    ...
    for idx=1:length(patches)
        number = length(patches(idx).blocksInfo);
