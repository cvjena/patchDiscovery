function visualizedImg = visualizePatchMeans ( varargin )
% function visualizedImg = visualizePatchMeans ( varargin )
% 
...
% 
% BRIEF :
%   Visualizes a patchMeanFeature by creating an image, drawing meanValues
%   in every block, and overlaying a grid.
% 
% INPUT :
%    feature       --  double array of patchMeanFeatures
...
%                      default: true
    ...
    if ( myInput.b_normalizeCells )
        ...
        normPatchMeans = sum(myInput.feature, 3);
        % avoid division by zero
        normPatchMeans(normPatchMeans < eps) = 1;
        % L1-normalize
        myInput.feature = bsxfun(@rdivide, myInput.feature, normPatchMeans);
    end
    ...
    [height, width, depth] = size ( myInput.feature ) ;
        
    imgPatches = zeros ( [myInput.heightOfCell*height, width*myInput.widthOfCell, depth], 'double' );
        ...
        for  i = 1:width
            for depthIt =1:depth
                imgPatches ( myInput.heightOfCell*(j-1) + 1:myInput.heightOfCell*(j), ...
                             myInput.widthOfCell*(i-1)  + 1:myInput.widthOfCell*(i), depthIt) ...
                             = myInput.feature(j,i,depthIt);
            end
        end
    end   
